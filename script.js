...
function initSmoothScrolling() {
    const navLinks = document.querySelectorAll('.nav-link, .learn-more-link, .btn-apply');
...
function initContactForm() {
    ...
    Object.keys(validators).forEach(fieldName => {
        ...
        if (field) {
            ...
            field.addEventListener('input', () => {
                if (form.querySelector(`[data-for="${fieldName}"]`).classList.contains('show')) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            statusEl.textContent = 'Message sent successfully! We\'ll get back to you via email soon.';
            statusEl.style.opacity = '1';
            
            // Reset form
            form.reset();
            Object.keys(validators).forEach(fieldName => clearFieldError(fieldName));
